...
class TdinfoParser(object):
    ...
    def _apply_type(self, symbol):
        ...
        self._apply_type_record(symbol_ea, symbol_name, type)

    def _apply_type_record(self, symbol_ea, symbol_name, type):
        if (type.id == tdinfo_structs.TypeId.SCHAR.name or
            ...
            if member.id == tdinfo_structs.TypeId.ARRAY.name: # array of arrays
                ...
                self._apply_type_record(symbol_ea, symbol_name, member)
            ...
            if get_struc_id(struct_name) == BADADDR: #check if struct already exists
                self._apply_struct(struct_name, type)
            idc.create_struct(symbol_ea, -1, struct_name)

    def _apply_struct(self, struct_name, type): #create struct + members
